...
export default async function getTranslations(cdnTranslationsUrl: string, plexyWebVersion: string, locale: string): Promise<Translations> {
    try {
        return await httpGet({
            loadingContext: cdnTranslationsUrl,
            errorLevel: 'fatal',
            errorMessage: `Translations: Failed to fetch translations for locale "${locale}"`,
            path: `sdk/${plexyWebVersion}/translations/${locale}.json`
        });
    } catch (error) {
        return await httpGet({
            loadingContext: cdnTranslationsUrl,
            ...
            path: `sdk/${plexyWebVersion}/translations/en-US.json`
        });
    }
}
