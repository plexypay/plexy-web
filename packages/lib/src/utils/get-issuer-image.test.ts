...
describe('Get issuer image', () => {
    describe('getIssuerImage', () => {
        ...
        test('Prepares options when there is an issuer ID', () => {
            expect(getIssuerImage(options, 'ideal', getImage)(issuer)).toBe('http://plexy.com/images/logos/ideal/123.svg');
            expect(getIssuerImage(options, 'test', getImage)(issuer)).toBe('http://plexy.com/images/logos/test/123.svg');
        });
