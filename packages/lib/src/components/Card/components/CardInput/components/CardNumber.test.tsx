...
describe('CardNumber and the (dual)branding icons that show in the PAN field', () => {
    test('Renders a CardNumber field, with standard brand image, and no dual branding', () => {
        ...
        expect(wrapper.find('.plexy-checkout__card__cardNumber__brandIcon')).toHaveLength(1);
        expect(wrapper.find('.plexy-checkout__card__dual-branding__icons')).toHaveLength(0);
    });
    ...
    test('Renders a CardNumber field with inline dual branding icons', () => {
        ...
        expect(wrapper.find('.plexy-checkout__card__dual-branding__icons')).toHaveLength(1);
    });

    test('Dual branding icons should consist of 2 images and should replace the standard brand image', () => {
        // dual branding icons
        expect(wrapper.find('.plexy-checkout__card__dual-branding__icons .plexy-checkout__card__cardNumber__brandIcon')).toHaveLength(2);
        // std brand image
        expect(wrapper.find('.plexy-checkout__card__cardNumber__input .plexy-checkout__card__cardNumber__brandIcon')).toHaveLength(0);
    });
