...
const CtPResendOtpLink = ({ onError, onResendCode, disabled }: CtPResendOtpLinkProps): h.JSX.Element => {
    ...
    if (showConfirmation) {
        return (
            <div className="plexy-checkout-ctp__otp-resend-code--confirmation">
    ...
    if (counter > 0) {
        return (
            <div className="plexy-checkout-ctp__otp-resend-code--disabled">
                {i18n.get('ctp.otp.resendCode')} -{' '}
                <span className="plexy-checkout-ctp__otp-resend-code-counter"> {counter > 0 && `${counter}s`} </span>
            </div>
        );
    }
