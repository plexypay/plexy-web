...
function Await(props: AwaitComponentProps) {
    ...
    const onComplete = (status: StatusObject): void => {
        ...
        setExpired(true);
        props.onError(new PlexyCheckoutError('ERROR', 'successful result, but no payload in response'));
    };
    ...
    const onError = (status: StatusObject): void => {
        ...
        props.onError(new PlexyCheckoutError('ERROR', 'error result with no payload in response'));
    };
    ...
    const finalState = (image: string, message: string) => (
        <div className="plexy-checkout__await plexy-checkout__await--result">
            <img
                className="plexy-checkout__await__icon plexy-checkout__await__icon--result"
                src={getImage({ imageFolder: 'components/' })(image)}
                alt={i18n.get(message)}
            />
            <p className="plexy-checkout__await__subtitle plexy-checkout__await__subtitle--result">{i18n.get(message)}</p>
        </div>
    );
    ...
    if (loading) {
        return (
            <div className="plexy-checkout__await">
                {props.brandLogo && <img src={props.brandLogo} alt={props.type} className="plexy-checkout__await__brand-logo" />}
    ...
    return (
        <div
            className={classnames(
                'plexy-checkout__await',
                `plexy-checkout__await--${props.type}`,
                props.classNameModifiers.map(m => `plexy-checkout__await--${m}`)
            )}
        >
            {props.brandLogo && <img src={props.brandLogo} alt={props.type} className="plexy-checkout__await__brand-logo" />}
            ...
            {!!(props.showAmount && amount?.value !== null && amount?.currency) && (
                <div className="plexy-checkout__await__amount">{i18n.amount(amount.value, amount.currency)}</div>
            )}

            {props.messageText != null && <div className="plexy-checkout__await__subtitle">{props.messageText}</div>}

            <div className="plexy-checkout__await__indicator-holder">
                <div className="plexy-checkout__await__indicator-spinner">
                    <Spinner inline={false} size="medium" />
                </div>
                <div className="plexy-checkout__await__indicator-text">{props.awaitText}</div>
            </div>

            {props.showCountdownTimer && (
                <div className="plexy-checkout__await__countdown-holder">
                    <div className="plexy-checkout__await__progress">
                        <span className="plexy-checkout__await__percentage" style={{ width: `${percentage}%` }} />
                    </div>

                    <div className="plexy-checkout__await__countdown">
                        {timeToPayString[0]}&nbsp;
            ...
            {props.url && !props.shouldRedirectAutomatically && (
                <div className="plexy-checkout__await__app-link">
                    <ContentSeparator />
            ...
            {props.instructions && (
                <div className="plexy-checkout__await__instructions">
                    {typeof props.instructions === 'string' ? i18n.get(props.instructions) : props.instructions?.()}
                </div>
            )}

            {props.endSlot && <div className="plexy-checkout__await__end-slot">{props.endSlot()}</div>}
        </div>
    );
}
