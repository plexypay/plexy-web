...
const UPIIntentAppItem = ({ app, imgSrc, isSelected, onSelect = () => {} }: UPIIntentAppItemProps): h.JSX.Element => {
    const buttonId = `plexy-checkout-upi-app-item-button-${app.id}`;
    const containerId = `plexy-checkout-upi-app-${app.id}`;
    ...
    return (
        ...
        <li
            className={cx({
                'plexy-checkout-upi-app-item': true,
                'plexy-checkout-upi-app-item--selected': isSelected
            })}
            onClick={() => handleAppSelected(app)}
        >
            <div className="plexy-checkout-upi-app-item-header">
                <ExpandButton classNameModifiers={['upi-app-item']} buttonId={buttonId} isSelected={isSelected} expandContentId={containerId}>
                    ...
                    <label className="plexy-checkout-upi-app-item__label" htmlFor={buttonId}>
                        {app.name}
                    </label>
                </ExpandButton>
            </div>
            {isSelected && <span className="plexy-checkout-upi-app-item__checkmark" />}
        </li>
    );
};
